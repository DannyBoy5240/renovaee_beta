...
shared ({caller = owner}) actor class IndexCanister() = this {
  ...
  public shared({caller = caller}) func autoVideoCanister(pk: Text): async Text {
    if (Utils.callingCanisterOwnsPK(caller, pkToCanisterMap, pk)) {
      Debug.print("creating an additional canister for pk=" # pk);
      await createVideoCanister(pk, ?[owner, Principal.fromActor(this)])
    } else {
      throw Error.reject("not authorized");
    };
  };
  ...
  public shared({caller = creator}) func createHelloServiceCanisterByGroup(userId: Text): async ?Text {
  ...
  public shared({caller = creator}) func createVideoCanisterByGroup(): async ?Text {
    let pk = "video";
    let canisterIds = getCanisterIdsIfExists(pk);
    if (canisterIds == []) {
      ?(await createVideoCanister(pk, ?[owner, Principal.fromActor(this)]));
  ...
  func createVideoCanister(pk: Text, controllers: ?[Principal]): async Text {
    Debug.print("creating new video canister with pk=" # pk);
    Cycles.add(300_000_000_000);
    let newVideoCanister = await Video.Video({
      partitionKey = pk;
      scalingOptions = {
        autoScalingHook = autoScaleHelloServiceCanister;
        sizeLimit = #heapSize(900_000_000); // Scale out at 475MB
        // for auto-scaling testing
        //sizeLimit = #count(3); // Scale out at 3 entities inserted
      };
      owners = controllers;
    });
    let newVideoCanisterPrincipal = Principal.fromActor(newVideoCanister);
    await CA.updateCanisterSettings({
      canisterId = newVideoCanisterPrincipal;
      settings = {
        controllers = controllers;
        compute_allocation = ?0;
        memory_allocation = ?0;
    ...
    let newVideoCanisterId = Principal.toText(newVideoCanisterPrincipal);
    
    pkToCanisterMap := CanisterMap.add(pkToCanisterMap, pk, newVideoCanisterId);

    Debug.print("new video canisterId=" # newVideoCanisterId);
    newVideoCanisterId;
  };
